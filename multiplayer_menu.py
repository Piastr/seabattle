...
def listen(s: socket.socket, host: str, port: int, members_on_server, game_on):
    while True:
        ...
        if msg.startswith('__attack'):
            ...
            i = pos[0]
            j = pos[1]
            game.pos_ships_enemy_attack[0] = int(i)
            game.pos_ships_enemy_attack[1] = int(j)

        if msg == '__change':
            game.my_turn = False

        if msg == '__yougotme':
            game.list_rects_enemy[game.pos_ships_mine_attack[0]][game.pos_ships_mine_attack[1]].alive = 1

        if msg == '__youkillme':
            game.list_rects_enemy[game.pos_ships_mine_attack[0]][
                game.pos_ships_mine_attack[1]].change()
            game.list_rects_enemy[game.pos_ships_mine_attack[0]][
                game.pos_ships_mine_attack[1]].alive = 1

        if msg == '__miss':
            game.list_rects_enemy[game.pos_ships_mine_attack[0]][game.pos_ships_mine_attack[1]].alive = 2

        if msg.startswith('__ready'):
            game.get_pos_enemy_ship = True
...
def multiplayer_menu(running, host, port):
    ...
    pygame.display.set_caption("multilayer_menu")
